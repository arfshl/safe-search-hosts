...
# IP Address for Ecosia safe search
#IPSix=$(dig strict.bing.com AAAA +short)
IPFour=$(curl --silent 'https://dns.google.com/resolve?name=strict-safe-search.ecosia.org&type=A' | jq -cr '.Answer[] | select(.type == 1) | .data')
...
# echo >> $output
echo "#$IPFour strict-safe-search.ecosia.org" >> $output
#echo "#$IPSix strict-safe-search.ecosia.org" >> $output
echo >> $output
